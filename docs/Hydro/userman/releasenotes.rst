...
#. **Geometry**: 
    a. Geometry can be imported as Bathymetry files (GeoClaw format - type 1); STL files or using the Hydro flume digital twin. 
    b. Shallow-water to CFD interface can be imported as a ``.csv`` file only.
    c. Buildings of cuboid shapes are supported in ``v1.0.0``. For additional shapes, the user can upload them as an STL file. The buildings need to be specified in the table or can be generated parametrically. Importing buildings as a ``.csv`` file is not currently supported in ``v1.0.0`` but can be requested using the :ref:`lbl-featurereq`. 
...
#. **Boundary conditions**: 
    ...
    c. Pressure boundary conditions include zeroGradient and fixedValue. Alternatively, the user can also leave the default option and an appropriate boundary condition relevant to the velocity boundary will be choosen.
