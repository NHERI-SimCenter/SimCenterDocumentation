...
buildings based on asset attibutes for the subjected hazard type (wind or storm surge). These attributess 
are the buildings properties or structure features that would influnece the 
...
in the available data. Taking the shutters of assets that was built before 2000, the Human Subjects Data 
indicates that roughtly 45% of houses had shutters on at some or all of their windows.  Accordingly, in the 
...
translational speeds, central presures, or handing angles (which is usually termed as epistemic uncertainty); 
...
For the epistemic uncertainty, multiple scenarios can be simulated using the sythetic simulation methods as 
...
For the aleatory uncertainty, this workflow adapts a nearest-neighbors method to propogate it in the workflow. 
whose idea is illustrated in :numref:`nearestneighbors`. The peak wind speed and sotrm surge values are simulated 
at grid sites (blue round dots), and for each seleted building asset location (red square dot), the workflow  randomly select 5 samples of 
intensity measure (wind speed and storm surge) from the 4 nearest neighbors (yellow circles). Note this random 
...
The testbed implemented :ref:`pelicun<https://pelicun.readthedocs.io/en/latest/index.html>` (Probabilistic Estimation of Losses, Injuires, and Community resilience Under 
...
for each asset in this testbed. Second, the corresponding damge and loss models are populated for each asset from HAZUS 
...
functions) fit to the synthetic data by maximizing the likehood of the observations assuming a Binomial distribution of 
