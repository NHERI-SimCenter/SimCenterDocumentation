...
:numref:`fig-RDTRESPanel` shows the results panel for the **Pelicun Damage and Loss** application. The results panel is comprised of several docking windows that can be moved, rearranged, resized, and opened and closed by the user. The results panel contains these main docks:
	...
	#. **Charts Dock:** Graphical respresentation of the results in three charts:
		...
		To switch between the charts, the user can click on the **Chart Selection Tabs** shown in :numref:`fig-RDTRESPanel`
	...
	#. **Detailed Building Results Dock:** Table showing the detailed building results. For each building, the table provides the:
		...
		The **Table Sort** combo box, shown in :numref:`fig-RDTRESPanel`, sorts the table in descending order according to the selected filter, e.g., repair cost. 
