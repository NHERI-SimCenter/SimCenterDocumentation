...
#. The ``Building Information`` table provides a user-editable spreadsheet of the information provided by the building inventory database. When a subset of buildings are selected for analysis, only the selected buildings will appear in the ``Building Information`` table. Pressing the ``Clear Selection`` button will clear the list of buildings that will be analysed and show all buildings in the ``Building Information`` table again. 
