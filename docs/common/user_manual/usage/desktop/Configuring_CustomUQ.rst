...
The quoFEM interface can be customized to accept the required inputs for user-specified UQ engines. The "CustomUQ" option should be selected, as shown in :numref:`CustomUQInput`. The UQ application name should then be input as "Other-UQ" as this will direct quoFEM to invoke the customized UQ backend where users can plug in their own functionality.	    
